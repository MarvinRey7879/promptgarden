/**
 * It. 168: Fügt die übersetzten 12 neuen Claude-Code-Befehle in die es/fr/zh-
...
const ERWARTET = ['bug', 'stop', 'team-onboarding', 'terminal-setup', 'theme', 'tui', 'ultrareview', 'usage-credits', 'voice', 'web-setup', 'workflows', 'pr-comments'];

for (const lang of ['es', 'fr', 'zh']) {
  const uebers = JSON.parse(readFileSync(`${SP}neue-${lang}.json`, 'utf8'));
  if (!Array.isArray(uebers) || uebers.length !== 12) throw new Error(`${lang}: keine 12 Objekte`);
  ...
  let neu = 0;
  for (const k of uebers) {
    if (daten.commands.some((x) => x.platform === 'claude-code' && x.slug === k.slug)) continue;
    daten.commands.push({ platform: 'claude-code', ...k });
    neu++;
  }
