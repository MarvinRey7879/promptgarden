...
import Klappbar from '@/components/Klappbar';

// Beschriftung der zusammenklappbaren Anbieter-Gruppen auf schmalen Bildschirmen
const MODELL_LABEL: Record<string, string> = {
  de: 'Modelle',
  en: 'models',
  es: 'modelos',
  fr: 'modèles',
...
export default async function VergleichePage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <p className="mono" style={{ margin: '0 0 26px', fontSize: 13, color: 'var(--muted)' }}>
      ...
      {data.ratio && (
        <div className="card" style={{ padding: '18px 22px', marginBottom: 34, background: '#fff', boxShadow: '4px 4px 0 var(--ink)' }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table className="ratio-tab" style={{ borderCollapse: 'collapse', width: '100%', minWidth: 560, fontSize: 14 }}>
              <thead>
                <tr>
                  {/* Sortierspalte (Ratio) steht direkt hinter dem Namen, damit sie auf
                      dem Handy sichtbar ist — vorher stand dort die unsortierte Index-Spalte */}
                  {[
                    { h: data.ratio.spalten.platz, mobil: true },
                    { h: data.ratio.spalten.modell, mobil: true },
                    { h: data.ratio.spalten.ratio, mobil: true },
                    { h: data.ratio.spalten.index, mobil: false },
                    { h: data.ratio.spalten.blended, mobil: false },
                  ].map(({ h, mobil }) => (
                    <th key={h} className={mobil ? undefined : 'hide-mobile'} style={{ textAlign: 'left', borderBottom: '3px solid var(--ink)', padding: '6px 10px', fontSize: 13, letterSpacing: '.05em' }}>{h.toUpperCase()}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {data.ratio.rows.map((r, i) => (
                  <tr key={r.name} style={{ background: i < 3 ? 'var(--lime)' : i % 2 ? 'var(--bg)' : '#fff' }}>
                    ...
                    <td className="hide-mobile" style={{ padding: '7px 10px', fontVariantNumeric: 'tabular-nums' }}>{r.index.toFixed(2)}</td>
                    <td className="hide-mobile" style={{ padding: '7px 10px', fontVariantNumeric: 'tabular-nums' }}>{r.blended}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <ul style={{ margin: '12px 0 0', paddingLeft: 18, fontSize: 13, color: 'var(--muted)', lineHeight: 1.6 }}>
        ...
        <div style={{ marginBottom: 34 }}>
          ...
          {data.modelle.anbieter.map((a, ai) => (
            <div key={a.name} style={{ marginBottom: 18 }}>
              <Klappbar
                titel={a.name.toUpperCase()}
                anzahl={a.modelle.length}
                offenLabel={MODELL_LABEL[lang]}
                standardOffen={ai === 0}
              >
                ...
                {a.modelle.map((m) => (
                  <div key={m.name} className="card" style={{ padding: '16px 18px', transform: `rotate(${PROVIDER_TILT[ai % 4]})`, boxShadow: '3px 3px 0 var(--ink)' }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
                      <span className="chip" style={{ fontSize: 12.5 }}>🧠 {m.kontext}</span>
                      <span className="chip" style={{ fontSize: 12.5, background: 'var(--yellow)' }}>💶 {m.preis}</span>
                    </div>
                    <ul style={{ margin: '0 0 8px', paddingLeft: 16, fontSize: 14, lineHeight: 1.55 }}>
                      {m.stark.map((s) => (
                        <li key={s}>✅ {s}</li>
                      ))}
                      {m.schwach.map((s) => (
                    ...
                    <p style={{ margin: 0, fontSize: 14, background: 'var(--lime)', border: '2px solid var(--ink)', borderRadius: 10, padding: '7px 10px', fontWeight: 700 }}>
                      → {m.wofuer}
                    </p>
                  </div>
                ))}
            ...
            {data.duelle.items.map((d, i) => (
              <div key={d.id} className="card" style={{ padding: '20px 24px', transform: `rotate(${i % 2 === 0 ? '-.25deg' : '.25deg'})`, boxShadow: '5px 5px 0 var(--ink)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
                  ...
                  <span className="mono" style={{ fontSize: 14, color: 'var(--muted)', fontWeight: 700 }}>{d.frage}</span>
                </div>
                ...
                {d.benchmark && (
                  <p style={{ margin: '0 0 8px', fontSize: 14, color: 'var(--muted)' }}>📊 {d.benchmark}</p>
                )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 16 }}>
            {data.imTool.tools.map((tl, i) => (
              <div key={tl.tool} className="card" style={{ padding: '18px 20px', transform: `rotate(${i % 2 === 0 ? '-.3deg' : '.3deg'})`, boxShadow: '4px 4px 0 var(--ink)' }}>
                ...
                <p style={{ margin: '0 0 10px', fontSize: 14, color: 'var(--muted)' }}>{tl.standard}</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10 }}>
                  {tl.empfehlungen.map((e) => (
                    <div key={e.aufgabe} style={{ border: '2px solid var(--ink)', borderRadius: 12, padding: '8px 12px', background: 'var(--bg)' }}>
                      <p style={{ margin: 0, fontSize: 14, fontWeight: 800 }}>{e.aufgabe}</p>
        ...
        {data.tools.map((tool, i) => (
          <div key={tool.id} className="card" style={{ padding: '22px 26px', transform: `rotate(${i % 2 === 0 ? '-.3' : '.3'}deg)` }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
              <span className="chip" style={{ fontSize: 12 }}>{tool.typ}</span>
              <span className="chip" style={{ fontSize: 12, background: tool.openSource === 'ja' || tool.openSource === 'yes' || tool.openSource === 'sí' || tool.openSource === 'oui' || tool.openSource === '是' ? 'var(--lime)' : 'var(--pink)' }}>
                Open Source: {tool.openSource}
              </span>
              <span className="chip" style={{ fontSize: 12, background: 'var(--yellow)' }}>{tool.preis}</span>
            </div>
