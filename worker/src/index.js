...
/**
 * Signatur für getrackte Mail-Links. Ohne sie wäre /v1/r ein offener Redirect —
 * damit könnte jeder unsere Domain für Phishing-Weiterleitungen missbrauchen.
 * Signiert wird das Ziel; der Redirect prüft die Signatur, bevor er weiterleitet.
 */
async function linkSig(secret, url) {
  const data = new TextEncoder().encode(`${secret}|${url}`);
  const h = await crypto.subtle.digest('SHA-256', data);
  return [...new Uint8Array(h)].slice(0, 8).map((b) => b.toString(16).padStart(2, '0')).join('');
}

/** Baut einen getrackten Link (fällt ohne Secret auf den direkten Link zurück). */
async function trackedUrl(secret, target, { campaign, item, lang, kind }) {
  if (!secret) return target;
  const sig = await linkSig(secret, target);
  const q = new URLSearchParams({ u: target, c: campaign, i: item || '', l: lang || '', k: kind, s: sig });
  return `${API_BASE}/v1/r?${q}`;
}
 ...
 */
export async function digestMailHtml(lang, items, unsubUrl, t, opts = {}) {
  const { secret = '', campaign = 'digest' } = opts;
  const site = `https://promptgarten.com/${lang}`;

  const karten = [];
  for (const i of items) {
    const tag = t.tags?.[i.tag] || i.tag || '';
    const farbe = TAG_FARBE[i.tag] || '#c9e265';
    const quelleRoh = Array.isArray(i.sources) && i.sources[0]?.url ? i.sources[0].url : '';
    // Primärziel ist die eigene Seite (Anker je Meldung existiert im Feed), nicht die Fremdquelle.
    const storyUrl = await trackedUrl(secret, `${site}/feed/#${i.id}`, { campaign, item: i.id, lang, kind: 'story' });
    const quelleUrl = quelleRoh ? await trackedUrl(secret, quelleRoh, { campaign, item: i.id, lang, kind: 'source' }) : '';
    karten.push(`
      <tr><td style="padding:0 0 16px">
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#ffffff;border:2px solid #2b2118;border-radius:14px">
          <tr><td style="padding:17px 20px 15px">
            <table role="presentation" cellpadding="0" cellspacing="0" border="0"><tr>
              <td style="background:${farbe};border:1.5px solid #2b2118;border-radius:7px;padding:2px 9px;font:700 10.5px/1.5 -apple-system,Segoe UI,sans-serif;color:#2b2118;text-transform:uppercase;letter-spacing:.07em">${escapeHtml(tag)}</td>
              <td style="padding-left:9px;font:400 11.5px/1.5 -apple-system,Segoe UI,sans-serif;color:#8a7d70">${escapeHtml(i.date || '')}</td>
            </tr></table>
            <p style="margin:11px 0 6px;font:800 17px/1.35 -apple-system,Segoe UI,sans-serif">
              <a href="${escapeHtml(storyUrl)}" style="color:#2b2118;text-decoration:none">${escapeHtml(i.title)}</a>
            </p>
            <p style="margin:0 0 13px;font:400 14px/1.65 -apple-system,Segoe UI,sans-serif;color:#4a4038">${escapeHtml(i.summary)}</p>
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-top:1px solid rgba(43,33,24,.13)"><tr>
              <td style="padding-top:11px">
                <a href="${escapeHtml(storyUrl)}" style="font:800 13px/1.4 -apple-system,Segoe UI,sans-serif;color:#e8613c;text-decoration:none">${escapeHtml(t.readOn)} &rarr;</a>
              </td>
              ${quelleUrl ? `<td align="right" style="padding-top:11px"><a href="${escapeHtml(quelleUrl)}" style="font:600 12.5px/1.4 -apple-system,Segoe UI,sans-serif;color:#8a7d70;text-decoration:underline">${escapeHtml(t.source)} &#8599;</a></td>` : ''}
            </tr></table>
          </td></tr>
        </table>
      </td></tr>`);
  ...
  const ctaUrl = await trackedUrl(secret, `${site}/feed/`, { campaign, item: 'cta', lang, kind: 'cta' });
  const siteUrl = await trackedUrl(secret, `${site}/`, { campaign, item: 'site', lang, kind: 'cta' });
  ...
  <tr><td align="center" style="padding:26px 14px 40px">
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:600px">
      <tr><td style="padding:0 2px 20px">
        ...
        <p style="margin:7px 0 0;font:700 12px/1.5 -apple-system,Segoe UI,sans-serif;color:#e8613c;text-transform:uppercase;letter-spacing:.1em">${escapeHtml(t.kicker)}</p>
        <p style="margin:7px 0 0;font:400 14.5px/1.6 -apple-system,Segoe UI,sans-serif;color:#7d7064">${escapeHtml(t.intro(items.length))}</p>
      </td></tr>
      ${karten.join('')}
      <tr><td align="center" style="padding:6px 0 4px">
        <a href="${escapeHtml(ctaUrl)}" style="display:inline-block;background:#e8613c;color:#ffffff;border:2px solid #2b2118;border-radius:999px;padding:13px 30px;font:800 15px/1 -apple-system,Segoe UI,sans-serif;text-decoration:none">${escapeHtml(t.cta)} &rarr;</a>
      </td></tr>
      <tr><td align="center" style="padding:12px 0 0">
        <p style="margin:0;font:400 12.5px/1.6 -apple-system,Segoe UI,sans-serif;color:#8a7d70">${escapeHtml(t.ctaSub)}</p>
      </td></tr>
      <tr><td style="padding:26px 2px 0;border-top:1px solid rgba(43,33,24,.18)">
        <p style="margin:15px 0 0;font:400 11.5px/1.6 -apple-system,Segoe UI,sans-serif;color:#8a7d70">${escapeHtml(t.why)}</p>
        <p style="margin:9px 0 0;font:400 11.5px/1.6 -apple-system,Segoe UI,sans-serif;color:#8a7d70">
          <a href="${escapeHtml(siteUrl)}" style="color:#8a7d70">promptgarten.com</a> &nbsp;·&nbsp;
...
const MAIL_TXT = {
  de: { confirmSub: 'Bitte bestätige deine Anmeldung — promptgarten', confirm: 'Klick zum Bestätigen deiner Newsletter-Anmeldung:', confirmBtn: 'Anmeldung bestätigen', digestSub: 'Deine KI-Woche — promptgarten', unsub: 'Abmelden',
    kicker: 'Deine KI-Woche', intro: (n) => `${n} ${n === 1 ? 'Meldung' : 'Meldungen'} aus der Welt der Coding-Agenten — jede mit Quelle.`, cta: 'Alle Meldungen ansehen', readOn: 'Auf promptgarten lesen', ctaSub: 'Jede Meldung mit Quelle — kostenlos, ohne Anmeldung.', source: 'Quelle', why: 'Du bekommst diese Mail, weil du dich auf promptgarten.com für den Newsletter angemeldet und die Anmeldung bestätigt hast.',
  ...
  en: { confirmSub: 'Please confirm your signup — promptgarten', confirm: 'Click to confirm your newsletter signup:', confirmBtn: 'Confirm signup', digestSub: 'Your AI week — promptgarten', unsub: 'Unsubscribe',
    kicker: 'Your AI week', intro: (n) => `${n} ${n === 1 ? 'story' : 'stories'} from the world of coding agents — each one sourced.`, cta: 'See all stories', readOn: 'Read on promptgarten', ctaSub: 'Every story sourced — free, no sign-up.', source: 'Source', why: 'You are getting this because you signed up for the newsletter on promptgarten.com and confirmed your address.',
  ...
  es: { confirmSub: 'Confirma tu suscripción — promptgarten', confirm: 'Haz clic para confirmar tu suscripción:', confirmBtn: 'Confirmar', digestSub: 'Tu semana de IA — promptgarten', unsub: 'Darse de baja',
    kicker: 'Tu semana de IA', intro: (n) => `${n} ${n === 1 ? 'noticia' : 'noticias'} del mundo de los agentes de programación — cada una con su fuente.`, cta: 'Ver todas las noticias', readOn: 'Leer en promptgarten', ctaSub: 'Cada noticia con su fuente — gratis, sin registro.', source: 'Fuente', why: 'Recibes este correo porque te suscribiste al boletín en promptgarten.com y confirmaste tu dirección.',
  ...
  fr: { confirmSub: 'Confirme ton inscription — promptgarten', confirm: 'Clique pour confirmer ton inscription :', confirmBtn: 'Confirmer', digestSub: 'Ta semaine IA — promptgarten', unsub: 'Se désinscrire',
    kicker: 'Ta semaine IA', intro: (n) => `${n} ${n === 1 ? 'actualité' : 'actualités'} du monde des agents de code — chacune sourcée.`, cta: 'Voir toutes les actualités', readOn: 'Lire sur promptgarten', ctaSub: 'Chaque actualité sourcée — gratuit, sans inscription.', source: 'Source', why: 'Tu reçois cet e-mail parce que tu t’es inscrit à la newsletter sur promptgarten.com et que tu as confirmé ton adresse.',
  ...
  zh: { confirmSub: '请确认订阅 — promptgarten', confirm: '点击确认你的订阅：', confirmBtn: '确认订阅', digestSub: '你的 AI 一周 — promptgarten', unsub: '退订',
    kicker: '你的 AI 一周', intro: (n) => `${n} 条来自编程智能体领域的消息——每条都附出处。`, cta: '查看全部消息', readOn: '在 promptgarten 上阅读', ctaSub: '每条消息都附出处——免费，无需注册。', source: '来源', why: '你收到这封邮件，是因为你在 promptgarten.com 订阅了新闻通讯并确认了邮箱地址。',
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // Getrackter Redirect aus Newsletter-Mails. Leitet NUR weiter, wenn die
      // Signatur zum Ziel passt — sonst wäre das ein offener Redirect.
      // Gezählt wird ohne Personenbezug: Kampagne, Meldung, Sprache, Linkart.
      if (request.method === 'GET' && path === '/v1/r') {
        const target = url.searchParams.get('u') || '';
        const sig = url.searchParams.get('s') || '';
        const secret = env.IP_SALT || '';
        let ziel;
        try {
          ziel = new URL(target);
        } catch {
        ...
        if (ziel.protocol !== 'https:') return new Response('bad protocol', { status: 400 });
        if (!secret || sig !== (await linkSig(secret, target))) {
          return new Response('bad signature', { status: 403 });
        }
        const day = new Date().toISOString().slice(0, 10);
        try {
          await env.DB.prepare(
            'INSERT INTO mail_clicks (day, campaign, item, lang, kind) VALUES (?, ?, ?, ?, ?)',
          )
            .bind(day, clip(url.searchParams.get('c'), 80) || 'digest', clip(url.searchParams.get('i'), 120), clip(url.searchParams.get('l'), 5), clip(url.searchParams.get('k'), 20) || 'story')
            .run();
        } catch {
          // Zählen darf den Klick nie blockieren — im Zweifel einfach weiterleiten.
        }
      ...
      if (request.method === 'GET' && path === '/v1/admin/summary') {
        ...
        const [bugs, feedback, notes, signups, views, topPaths, forumRecent, forumBlocked, todos, viewsByDay, viewsByLang, viewsByCountry, topRefs, viewsTotal, signupsRecent, donations, revenue, visitors7d, internal7d, mailClicks, mailClicksTop] = await Promise.all([
          ...
          env.DB.prepare("SELECT COUNT(*) AS n FROM page_views WHERE internal=1 AND day >= date('now','-7 day')").first(),
                  env.DB.prepare("SELECT COUNT(*) AS n FROM mail_clicks WHERE day >= date('now','-30 day')").first(),
          env.DB.prepare("SELECT campaign, kind, item, COUNT(*) AS n FROM mail_clicks WHERE day >= date('now','-90 day') GROUP BY campaign, kind, item ORDER BY n DESC LIMIT 20").all(),
]);
        return json(
          {
            ...
            mail_clicks_30d: mailClicks?.n ?? 0,
            mail_clicks_top: mailClicksTop?.results ?? [],
  ...
  async scheduled(event, env) {
    ...
    for (const s of subs.results) {
      ...
      const html = await digestMailHtml(lang, items, unsubUrl, t, { secret: env.IP_SALT || '', campaign: 'digest-' + new Date().toISOString().slice(0,10) });
